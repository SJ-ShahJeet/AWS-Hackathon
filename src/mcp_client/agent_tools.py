...
from livekit.agents import AgentSession, ChatContext, JobContext
from livekit.agents import FunctionTool as Tool
...
class MCPToolsIntegration:
    ...
    def _create_decorated_tool(tool: FunctionTool) -> Callable:
        ...
        for p_name, p_details in schema_props.items():
            ...
            default = (
                ...
                else p_details.get("default", None)
            )
            params.append(
                inspect.Parameter(
                    name=p_name,
    ...
    async def register_with_agent(
        ...
        if hasattr(agent, "add_tool"):
            ...
            logger.warning("Agent does not have an 'add_tool' method, tools were not registered")
    ...
    async def create_agent_with_tools(
        ...
        # Prepare tools first, so we can pass them into the Agent constructor
        prepared_tools = await MCPToolsIntegration.prepare_dynamic_tools(
            mcp_servers,
            convert_schemas_to_strict=convert_schemas_to_strict,
            auto_connect=False,  # Already connected above
        )

        # Merge any pre-specified tools with MCP-prepared tools
        agent_kwargs = agent_kwargs or {}
        existing_tools = agent_kwargs.get("tools", []) or []
        all_tools = [*existing_tools, *prepared_tools]
        if all_tools:
            agent_kwargs["tools"] = all_tools

        # Create agent instance with full toolset
        agent = agent_class(**agent_kwargs)

        if prepared_tools:
            tool_names = [getattr(t, "__name__", "unknown") for t in prepared_tools]
            logger.info(
                f"Registered {len(prepared_tools)} MCP tools with agent via constructor: {tool_names}"
            )
        else:
            logger.warning("No MCP tools were found to register with the agent")
